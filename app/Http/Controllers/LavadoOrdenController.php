...
class LavadoOrdenController extends Controller
{
    ...
    public function cambiarEstado(Request $request, LavadoOrden $lavado)
    {
        $request->validate([
            'estado' => [
                'required', 'in:Pendiente,En proceso,Finalizado',
                function ($attribute, $value, $fail) use ($lavado) {
                    if ($value !== $lavado->estado && !in_array($value, $this->transicionesValidas[$lavado->estado] ?? [])) {
    ...
    {
        LavadoOrden::withTrashed()->findOrFail($id)->forceDelete();
    ...
    public function cobrar(Request $request, LavadoOrden $lavado)
    {
        if ($lavado->saldo <= 0) {
            return redirect()->route('lavados.index')
                ->with('info', 'El lavado #' . $lavado->id_orden . ' ya está saldado.');
        }

        $rules = ['metodo_pago' => 'required|in:efectivo,qr,efectivo/qr'];
        ...
        $request->validate($rules, [
            ...
            'monto_abono.max'      => 'El abono no puede superar el saldo de Bs. ' . number_format($lavado->saldo, 2) . '.',
        ...
        if ($request->metodo_pago === 'efectivo/qr') {
            ...
            if ($montoAbono <= 0 || $montoAbono > $lavado->saldo) {
                return back()->withInput()->withErrors([
                    'monto_efectivo' => 'La suma efectivo + QR debe ser > 0 y no superar el saldo.',
                ]);
            }
        } else {
        ...
        if ($nuevoMontoPagado >= $lavado->precio_total) $estadoPago = 'pagado';
        elseif ($nuevoMontoPagado > 0)                  $estadoPago = 'parcial';

        $estadoAnterior = $lavado->estado;
        $nuevoEstado    = $estadoAnterior;
        if ($saldo <= 0 && $lavado->estado !== 'Finalizado') $nuevoEstado = 'Finalizado';
        ...
        if ($nuevoEstado !== $estadoAnterior) $updateData['estado'] = $nuevoEstado;

        $lavado->update($updateData);

        $observacion = 'Cobro de Bs. ' . number_format($montoAbono, 2) . ' vía ' . $request->metodo_pago;
        if ($nuevoEstado !== $estadoAnterior) $observacion .= ' (Pago completado → Lavado finalizado)';
        ...
        if ($saldo <= 0) {
            return redirect()->route('lavados.index')
                ->with('success', '¡Pago completado! Lavado #' . $lavado->id_orden . ' finalizado.');
        }

        return redirect()->route('lavados.index')
            ->with('success', 'Cobro registrado. Saldo restante: Bs. ' . number_format($saldo, 2));
    }
